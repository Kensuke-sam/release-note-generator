...
- 何を解くか: リリース情報共有の作成コストを下げつつ、運用リスクを明確化する。
- 何を実装したか: デプロイ情報保存、ノート草案生成、ロールバック方針提示を API 化した。
- 学生としての強み: 開発生産性の題材でも、運用観点とレビュー前提の設計を入れている。

## 面接での60秒説明テンプレ

1. 課題: デプロイ後の共有文章作成が遅く、品質も担当者依存になりやすい。  
2. 設計: 変更入力と配布文面を分離し、草案を何度でも再生成できるようにした。  
3. 技術選択: AIは構造化出力のみ保存し、運用で再利用しやすいデータにした。  
4. 成果: 開発速度と運用の安全性を両立する、実務寄りの情報共有基盤を示せた。
